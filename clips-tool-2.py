import os, re, random, subprocess, sys, time
from pathlib import Path

# =========================
# ⚙️ CẤU HÌNH
# =========================
SCENE_THRESHOLD = 0.40       # 0.30–0.60 tuỳ nguồn
MIN_DURATION    = 4.0        # chỉ giữ scene >= 4s
...
def pick_hwaccel_and_dec(input_path: str):
    """Chọn hwaccel tốt nhất theo codec nguồn (ưu tiên CUDA/NVDEC)."""
    codec = "unknown"
    try:
...
def atempo_chain(speed: float) -> str:
    s = 1.0 if speed <= 0 else float(speed)
    chain = []
    while s > 2.0:
...
def detect_scenes_ffmpeg(input_path: str, thr: float, min_duration: float) -> list[tuple[float,float]]:
    ...
    # Nếu decode bằng CUDA, trước khi dùng filter CPU phải hwdownload
    ...
    cmd = (["ffmpeg","-hide_banner","-an",
            "-probesize", PROBESIZE, "-analyzeduration", ANALYZE_DUR] +
...
# =========================
# CẮT CLIP + ENCODE (NVENC nếu có)
# =========================
def cut_one_pass(input_path: str, start: float, end: float,
                 out_path: str, speed: float = 1.0):
    hwaccel_args, vdec_override, using_cuda = pick_hwaccel_and_dec(input_path)

    # VIDEO chain: nếu CUDA → hwdownload,format=nv12 trước setpts
    vpre = "hwdownload,format=nv12," if using_cuda else ""
    vf  = f"{vpre}setpts=PTS/{speed}"

    # AUDIO chain: gộp atempo + aresample vào cùng -filter_complex (không dùng -af)
    af  = f"{atempo_chain(speed)},aresample=async=1:first_pts=0"

    if _has("encoder", "h264_nvenc"):
        vcodec = ["-c:v","h264_nvenc","-preset",ENCODE_PRESET,
                  "-rc","constqp","-qp",str(QP_VALUE),
    ...
    # Ghi stderr vào .err.txt để debug khi lỗi

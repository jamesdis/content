...
def filter_links_by_duration(urls_file: Path, out_file: Path, min_sec: int, max_sec: int) -> dict:
    ...
    # yt-dlp match-filter: dùng 'and' (không dùng '&')
    match_filter = f"duration >= {min_sec} and duration <= {max_sec}"

    # lấy cookies để TikTok bớt lỗi
    cookie_args = detect_browser_cookies()

    # Gọi yt-dlp: skip download, chỉ in các link thỏa điều kiện
    # Lưu ý: --print %(webpage_url)s để luôn nhận URL sạch
    cmd = [
        "yt-dlp",
        "--ignore-errors",
        "--no-warnings",
    ...
    # Thu thập link hợp lệ (1 link/line)
    lines = (proc.stdout or "").splitlines()
    valid = [ln.strip() for ln in lines if ln.strip()]
    ...
    # Đếm tổng số link đầu vào (không tính dòng trống/comment)
    total_input = 0
